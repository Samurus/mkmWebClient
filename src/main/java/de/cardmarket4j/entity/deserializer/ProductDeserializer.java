  ...
  public Product deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    if (jObject.keySet().contains("localization")) {
      for (JsonElement jElement : jObject.get("localization").getAsJsonArray()) {
        ...
        mapLocalizedNames
            .put(CardMarketUtils.fromLanguageId(JsonIO.parseInteger(jLoc, "idLanguage")),
    ...
    return new Product(productId, metaproductId, totalReprints, name, mapLocalizedNames, categoryId,
        categoryName,
        selfUrl, imageUrl, game, expansionCollectionNumber, rarity, expansionName, expansion,
        priceGuide,
