...
public class AccountDeserializer implements JsonDeserializer<Account> {
  ...
  public Account deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    BigDecimal totalBalance = JsonIO
        .parseBigDecimal(jObject.get("moneyDetails").getAsJsonObject(), "totalBalance");
    BigDecimal moneyBalance = JsonIO
        .parseBigDecimal(jObject.get("moneyDetails").getAsJsonObject(), "moneyBalance");
    BigDecimal bonusBalance = JsonIO
        .parseBigDecimal(jObject.get("moneyDetails").getAsJsonObject(), "bonusBalance");
    BigDecimal unpaidAmount = JsonIO
        .parseBigDecimal(jObject.get("moneyDetails").getAsJsonObject(), "unpaidAmount");
    BigDecimal providerRechargeAmount = JsonIO
        .parseBigDecimal(jObject.get("moneyDetails").getAsJsonObject(),
            "providerRechargeAmounT");
    BankAccount bankAccount = JsonIO.getGson()
        .fromJson(jObject.get("bankAccount"), BankAccount.class);
    ...
    return new Account(user.getUserId(), user.getUserName(), user.getRegistrationDate(),
        user.getUserType(),
        user.isSeller(), user.getCompanyName(), user.getFirstName(), user.getLastName(),
        user.getAddress(),
        user.getPhoneNumber(), user.getEmailAddress(), user.getVat(), user.getRiskGroup(),
        user.getReputation(),
        user.getExpectedDeliveryTime(), user.getAmountSales(), user.getAmountSoldItems(),
        user.getAverageShippingTime(), user.isOnVacation(), country, maySell,
