  ...
  public Article deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    try {
      languageCode = CardMarketUtils
          .fromLanguageId(
              JsonIO.parseInteger(jObject.get("language").getAsJsonObject(), "idLanguage"));
    ...
    LocalDateTime lastEdited = JsonIO
        .parseLocalDateTime(jObject, "lastEdited", DateTimeFormatter.ISO_DATE_TIME);
    ...
    return new Article(articleId, productId, languageCode, comment, price, quantity, inShoppingCart,
        product,
