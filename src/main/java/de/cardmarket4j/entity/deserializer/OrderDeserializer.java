...
public class OrderDeserializer implements JsonDeserializer<Order> {
  ...
  public Order deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    LocalDateTime dateBought = JsonIO
        .parseLocalDateTime(jState, "dateBought", DateTimeFormatter.ISO_DATE_TIME);
    LocalDateTime datePaid = JsonIO
        .parseLocalDateTime(jState, "datePaid", DateTimeFormatter.ISO_DATE_TIME);
    LocalDateTime dateSent = JsonIO
        .parseLocalDateTime(jState, "dateSent", DateTimeFormatter.ISO_DATE_TIME);
    LocalDateTime dateReceived = JsonIO
        .parseLocalDateTime(jState, "dateReceived", DateTimeFormatter.ISO_DATE_TIME);
    LocalDateTime dateCanceled = JsonIO
        .parseLocalDateTime(jState, "dateCanceled", DateTimeFormatter.ISO_DATE_TIME);
    String cancellationReason = JsonIO.parseString(jState, "reason");
    Integer mergedOrderId = JsonIO.parseInteger(jState, "wasMergedInto");

    ShippingMethod shippingMethod = JsonIO.getGson()
        .fromJson(jObject.get("shippingMethod"), ShippingMethod.class);
    String trackingNumber = JsonIO.parseString(jObject, "trackingNumber");
    boolean isPresale = JsonIO.parseBoolean(jObject, "isPresale");
    Address shippingAddress = JsonIO.getGson()
        .fromJson(jObject.get("shippingAddress"), Address.class);
    ...
    return new Order(orderId, isBuyer, seller, buyer, orderState, dateBought, datePaid, dateSent,
        dateReceived,
        dateCanceled, cancellationReason, mergedOrderId, shippingMethod, trackingNumber, isPresale,
        shippingAddress, amountItems, note, evaluation, listArticles, listArticlesValue,
        serviceFeeValue,
        totalValue);
  }
