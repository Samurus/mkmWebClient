...
public class CardMarketService {
  ...
  public CardMarketService(String appToken, String appSecret, String accessToken,
      String accessTokenSecret) {
    this.authenticationService = new AuthenticationService(appToken, appSecret, accessToken,
        accessTokenSecret);
  ...
  Pair<Integer, JsonElement> legacyRequest(String URL, HTTPMethod httpMethod) throws IOException {
    try {
      ...
      BufferedReader rd = new BufferedReader(new InputStreamReader(
          (responseCode == 200 || responseCode == 206 || responseCode == 204) ? connection
              .getInputStream()
  ...
  Pair<Integer, JsonElement> request(String URL, HTTPMethod httpMethod, String requestBody)
      throws IOException {
    try {
      String uri = (sandBoxMode ? URI_SANDBOX : URI) + URL.replaceAll("\\s", "%20");
      HttpURLConnection connection = (HttpURLConnection) new URL(uri).openConnection();
